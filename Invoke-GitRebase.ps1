function Invoke-GitRebase
{
    <#
        .SYNOPSIS
        Performs a rebase. Aliased to `rebase`.
    ...
    if (-not $FromRef)
    {
        ...
        {
            $FromRef = Get-GitLog -SinceLastPRMerge | Select-Object -Last 1 -ExpandProperty Id
            if (-not $FromRef)
            {
    ...
    try
    {
        ...
        # fixes dodgy colour rendering
        # linux cli tools tend to pipe utf8 at you
        $ConsoleCodepage = [Console]::OutputEncoding
    ...
    finally
    {
        ...
        # Setting back to original still breaks some prompts (oh-my-posh).
        # This is why: https://github.com/microsoft/terminal/issues/4921#issuecomment-599135352
        # Solution: sleep long enough for git for windows to sort itself out
        if (
            $PSVersionTable.PSVersion.Major -gt 5 -and
        ...
        [Console]::OutputEncoding = $ConsoleCodepage


        Show-GitActionSummary rebase -Commits ""
    }
}
